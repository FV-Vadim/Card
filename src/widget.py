...
def mask_account_card(name: str, number_card: int) -> str:
    ...
    if len(str(number_card)) > 16:
        ...
        number_card = get_mask_account(number_card) # шифрует номер счета

    else:

        number_card = get_mask_card_number(number_card) # шифрует номер карты
