...
def get_mask_card_number(number_card: Union[int, str]) -> str:
    ...
    try:
        ...
        coder_number_card = ""
        if (
            number_card is None or not number_card
        ):  # Если в номер карты передается пустой список или ничего не передается
            coder_number_card = "0"
        ...
        logger.error(f"Ошибка при кодировании номера счета: {str(exception)}")
        return f"Синтаксическая ошибка: {str(exception)}"
...
def get_mask_account(number_card: Union[int, str]) -> str:
    ...
    try:
        ...
        if (
            ...
            16 <= len(str(number_card)) <= 24
        ):  # Если количество символов больше 16 и меньше 24 символов с учетом пробела
